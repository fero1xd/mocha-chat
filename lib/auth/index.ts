    ...
    after: createAuthMiddleware(async (ctx) => {
      ...
      if (
        ...
        const sessionToken = cookies.find((c) =>
          c.startsWith(sessionTokenName)
        );
        const sessionValue = sessionToken?.slice(
          sessionTokenName.length + 1,
          sessionToken.indexOf(";")
        );

        const { data, error } = await betterFetch<{ token: string }>(
          `${
            process.env.NODE_ENV === "development" ? "http" : "https"
          }://${process.env.NEXT_PUBLIC_VERCEL_PROJECT_PRODUCTION_URL}/api/auth/token`,
          {
            headers: {
